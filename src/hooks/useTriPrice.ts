import { ChainId, Fraction, JSBI } from '@ufomoney/newsdk';
...
export default function useTriPrice(): Result {
    ...
    const [
        wnearUsdcContractReserves,
        wnearTriContractReserves
    ] = [wnearUsdcContractCall.result, wnearTriContractCall.result];
    ...
    const getTriPrice = useCallback(() => {
        ...
        const usdcToWnearRatio = new Fraction(
            normalizedUSDCRatio,
            wnearUSDCPairReserves__wnear.toString(),
        );

        // TRI/NEAR
        const WnearToTriRatio = new Fraction(
            wnearTriPairReserves__tri.toString(),
            wnearTriPairReserves__wnear.toString(),
        );
    ...
    const triPriceFriendly = useMemo(
        () => getTriPrice()?.toFixed(2) ?? null,
        [getTriPrice, latestBlock],
    );
