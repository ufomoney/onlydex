...
import { Fraction, JSBI, TokenAmount } from '@ufomoney/newsdk';
...
export default function CirculatingSupplyMarketCap({
    totalTRI,
    triPrice,
}: Props) {
    ...
    const circulating = useMemo(() => isLoading
        ? null
        : JSBI.subtract(JSBI.BigInt(totalTRI!.toFixed(0)), LOCKED_TRI)
        , [isLoading, totalTRI, LOCKED_TRI]);

    const marketCap = useMemo(() => isLoading
        ? null
    ...
    const marketCapFriendly = isLoading
        ? '-'
        : `$${addCommasToNumber(marketCap!)}`;
